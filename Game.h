...
public:
	...
	void SetGame()
	{
		...
		cout << "                                           1)Easy" << endl
			<< "                                           2)Normal" << endl
			<< "                                           3)Hard" << endl
			<< "                                           4)Very very very very very hard" << endl;
		while (true)
		{
			chooseLevel = _getch();
			if (chooseLevel == '1' || chooseLevel == '2' || chooseLevel == '3' || chooseLevel == '4' )
			{
				level = chooseLevel - '0';
				switch (level)
				{
	...
	void UpdateScore(bool eaten)
	{
		if (eaten)
		{
			...
			for (int s = score; s != 0; s /= 10)    // 补全空格保证显示格式
				spaceUsed++;
	...
	void RandomFood(CFood& food)
	{
		...
		int x, y;
		while (true)  // 死循环，直到得到xy既在合理范围内同时不在蛇身上
		{
			x = rand() % (length / 2) * 2;      //  保证显示位置正确
			y = rand() % (width / 2) * 2;
